...
if st.session_state.logs and st.session_state.analysis_result:
    ...
    st.markdown('</div>', unsafe_allow_html=True)
"><b>PRIMARY WORKFLOW — EVIDENCE INTAKE</b> &nbsp; Upload → Preserve → Analyse → Investigate → Report. Start here for local security logs; accepted evidence is SHA-256 preserved before analysis.</div>', unsafe_allow_html=True)