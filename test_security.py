...
def test_parser_rejects_invalid_ips_without_crashing():
    event = parse_line(
        "2026-09-22T10:00:00+00:00 user=admin from 999.999.999.999 to 10.0.0.5 pid=42"
    )
...
def main():
    tests = [
        test_sanitization_redacts_secrets_and_bounds_input,
        test_parser_rejects_invalid_ips_without_crashing,
        test_parser_handles_malformed_untrusted_input,
        test_correlation_escalates_incident_to_highest_severity,
    ]
    for test in tests:
        test()
